...
Player Browser — Android WebView 기반 브라우저. URL 탐색 + 동영상 제스처 컨트롤 + 동영상 이어보기 + 즐겨찾기/방문기록 + 멀티탭(썸네일 갤러리/그룹/그룹순서변경/멀티선택/부모복귀/카드메뉴 그룹 이동/드래그 그룹 편집/바 스와이프 탭 전환) + 광고 차단 + 쿠키 동의 배너 자동 거부 + SNI 우회 + 프라이빗 DNS(DoH) + URL 숫자 자동/수동 복구 + 링크 항상 새 탭 열기 + Chromecast + 자체 업데이트 + 크래시 로깅. 현재 버전: v1.3.39.
...
- **풀스크린(Kotlin `GestureCapturingFrame`)은 단일 제스처 소스 — raw 터치를 자식에 전혀 안 흘림(v1.3.39):** `dispatchTouchEvent`가 모든 터치를 **완전 소비(`return true`)** 하고 `super.dispatchTouchEvent`로 자식 WebView/네이티브 `<video>`에 절대 전달하지 않음. 이전에는 탭 `ACTION_UP`만 `ACTION_CANCEL`로 치환하고 DOWN/MOVE는 그대로 흘렸는데, 그러면 사이트 플레이어가 그 `touchstart`들로 **자기 더블탭/스크럽을 함께 인식**해 우리 제스처와 충돌(이중 시킹·예기치 않은 풀스크린 토글) — 더블탭 판정 기준(click vs touchstart vs pointer)이 사이트마다 달라서 어떤 사이트는 멀쩡하고 어떤 사이트는 깨지던 "균일하지 않음"의 원인. 이제 모든 의도는 `window.__pb.*` 훅으로만 구동: 탭/더블탭은 `GestureDetector.onSingleTapConfirmed`/`onDoubleTap`(사이드 시킹), 스크럽/볼륨/밝기/`switchVideo`는 `dispatchTouchEvent` 분기. **사이트 오버레이 툴바(재생/⏩/확대/풀스크린종료 등) 버튼은 raw 터치 없이도 동작** — 단일탭/중앙 더블탭이 `togglePlay`를 직접 안 부르고 `window.__pb.fsTap(xRatio, yRatio)`로 위임(0~1 비율 → JS가 `innerWidth/Height`로 CSS 좌표 환산 → in-document와 **동일한 `playPauseAtPoint`**)해서, 버튼 위면 그 요소에 **합성 클릭**(`dispatchSyntheticClick`, isTrusted=false) 포워딩, 맨 비디오면 재생/정지 토글. JS in-document 경로는 `fsActive`로 이미 꺼져 있음. **트레이드오프:** 네이티브 `<video controls>`의 기본 컨트롤 바 자동 표시/네이티브 스크러버 드래그는 raw 터치를 못 받아 안 뜸 — 우리 스크럽 제스처가 대체. **한계:** 이 단일소스화는 `onShowCustomView`를 거치는 *네이티브* 풀스크린에만 적용 — 사이트가 CSS로 뷰포트를 채우는 *인-페이지* 풀스크린은 `fsActive=false`라 in-document(passive) 경로라 사이트 터치를 완전히 막지 못함(별도 작업 필요).
