...
- **링크 롱프레스 메뉴** — 페이지의 링크를 길게 누르면 "새 탭에서 열기 / 백그라운드 탭으로 열기 / 링크 주소 복사" 메뉴가 뜸. 전역 토글을 켜고 끌 필요 없이 원하는 링크만 새 탭에서 열 수 있음. "백그라운드 탭"은 현재 페이지에 머문 채 새 탭을 뒤에 쌓음
...
- **영상 롱프레스로 외부 플레이어 연결 (v1.3.50)** — 한 페이지에 영상이 여러 개일 때 화면 버튼만으로는 어떤 영상이 열리는지 알 수 없어서, 원하는 `<video>`를 **길게 누르면 바로 그 영상**을 외부 플레이어로 연결. 그 영상의 실제 소스를 읽어 재생하고, `blob:`/MSE라 URL이 없으면 감지된 네트워크 스트림으로 폴백
