...
```
app/src/main/
  assets/video_gestures.js        # 제스처 JS + initInlinePlayer(PBInline 보고/take·release) + initBodySniff(fetch/XHR 응답 앞 512B 판별 → PBPlayer.onStreamBody) + initFullscreenFit(풀스크린 영상 contain 최대화) + initResume + initExternalPlayer(영상 롱프레스) + initStallWatch(8초 무진행이면 대신 탐색, `[stall:net|buf #n]`/`[stall:giveup]` 토스트)
...
메인 세션은 Fable(`/model claude-fable-5-1`)이 설계·판단·마무리(버전 bump → README/CLAUDE.md(+필요 시 `docs/HISTORY.md`) → 커밋 → `v1.3.x` 태그 푸시)를 맡는다. **구현·검토를 누가 하는지는 메인이 작업 크기로 고르고, 요청을 받은 첫 줄에 "이 건은 ②로 갑니다" 처럼 한 줄로 알린다.** 사용자는 다르게 하고 싶을 때만 "①로 해" 식으로 바꾼다.

| | 작업 | 흐름 | 이유 |
|---|---|---|---|
| ① | 사소한 수정 — 한두 파일, 규칙 위반 위험 없음(문구·상수·토스트 코드·설정 항목) | **메인 혼자** | 핸드오프 비용이 작업보다 크다 |
| ② | 보통 기능 — 파일 몇 개, 아래 규칙 항목에 닿음 | **메인 구현 → `opus-reviewer` 검토** | 속도 거의 그대로, 검토 이득은 가져간다 |
| ③ | 큰 기능·위험 영역 — 파일 5개 이상, JS+Kotlin 양쪽, 캡차/인터셉트 체인/인라인 플레이어 상태 머신 | **메인 계획 → `sonnet-dev` 구현 → `opus-reviewer` 검토** | 토큰이 많이 나가는 구간이라 Sonnet 단가가 의미 있고, 메인 컨텍스트도 지킨다 |

- **계획:** ②③ 모두 메인이 건드릴 파일과 관련 규칙까지 적은 **구체적 계획**을 먼저 쓴다. ③ 은 그 계획을 그대로 `sonnet-dev` 프롬프트로 넘긴다 — 서브에이전트는 넘겨준 프롬프트만 알고 시작한다.
- **`sonnet-dev`** (`.claude/agents/sonnet-dev.md`) — 계획대로만 구현. 버전/커밋/문서는 안 건드림.
- **`opus-reviewer`** (`.claude/agents/opus-reviewer.md`) — diff 를 계획과 대조, 아래 규칙 체크리스트, CRITICAL/HIGH 직접 수정. (전역 `code-reviewer` 는 Edit 권한이 없어 안 씀)
- 판단이 애매하면 한 단계 위(②↔③)로 — 검토를 빼는 쪽보다 붙이는 쪽이 싸다(실기기 테스트 한 바퀴가 가장 비싼 비용이다).
...
- 서브리소스 풀은 **32개/90초**(v1.3.108) — HLS 사이트가 2초 조각을 13개 CDN 호스트에 라운드로빈하면 8개 풀은 조각마다 새 TLS 핸드셰이크를 만든다(tvmon1 무한 로딩). 메인 프레임 풀(4/5초)은 키우지 말 것(RST 된 커넥션이 다음 내비게이션을 오염). 가로챈 요청에서 `If-None-Match`/`If-Modified-Since` 는 뺀다 — 304 는 `WebResourceResponse` 가 거부한다(3xx 전부). 3xx 응답은 예외가 아니라 명시적으로 `close()` + `null`(네이티브가 따라간다) — 3xx 를 `WebResourceResponse` 에 넣으려 하지 말 것.
...
- 리시버가 로드를 받아들이기(`acceptedSeq`) 전의 `onMediaError`·IDLE/ERROR 는 토스트하지 말 것 — 이전 미디어 몫일 수 있고(빠른 두 번째 전송의 `LOAD_INTERRUPTED 904`), 새 로드 탓으로 돌리면 그 로드의 진짜 실패 토스트까지 막힌다. 수락 전 실패는 결과 콜백의 `[cast:load:<status>:<detailed>]` 가 보고한다.
...
- 멈춤 감시(`initStallWatch`, v1.3.107): 재생 중 8초 무진행이면 탐색으로 풀고 `[stall:net|buf #n]` 토스트, 3회 뒤 `[stall:giveup]`. 정지·페이지가 시작한 탐색·길이 미확정·끝 2초 전은 건드리지 않는다. **잡지 말 것:** `eligible()` 에 `!seeking` 을 다시 넣지 말 것(우리 점프 뒤 MSE 가 `seeking` 을 유지해 2회차·포기가 죽는다); 요소의 `__pbStallLease` 를 빼지 말 것(같은 출처 프레임 둘이 한 영상을 영원히 밀어 보낸다); `runFsGesture` seek/switch 의 `quiet` 를 빼지 말 것(풀스크린 HUD 와 DOM 토스트가 겹친다). `net` 이 반복 보고되면 다음 판은 격리 페이지의 미디어 요청 광고 차단 면제(v1.3.88 사다리 2/3).
