...
- **영상 롱프레스 → 메뉴로 외부 플레이어 연결 (v1.3.50, v1.3.51)** — 한 페이지에 영상이 여러 개일 때 화면 버튼만으로는 어떤 영상이 열리는지 알 수 없어서, 원하는 `<video>`를 **길게 누르면** 그 영상에 대한 메뉴("외부 플레이어로 재생 / 다운로드 / 영상 주소 복사")가 떠서 선택. 그 영상의 실제 소스를 읽어 재생하고, `blob:`/MSE라 URL이 없으면 감지된 네트워크 스트림으로 폴백. 재생할 스트림을 못 찾으면 메뉴 대신 안내만 뜸(v1.3.51 — 확인 없이 바로 실행되던 동작을 메뉴로 바꿈)
